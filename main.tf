...
resource "null_resource" "ilbIP" {
  ...
  provisioner "local-exec" {
    command = "env AZURE_CONFIG_DIR=/opt/jenkins/.azure-sandbox bash -e az resource show --ids ${azurerm_template_deployment.app_service_plan.outputs["aseResourceID"]}/capacities/virtualip --query internalIpAddress | tr -d \" > ${path.module}/ip.txt"
  }
}
